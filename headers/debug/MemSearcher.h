...
#include<cmath>
#include<typeinfo>
template<typename pattern_type,unsigned int pattern_sz,unsigned int hint,unsigned int MAXSZ,unsigned int Search_Off=128>
struct MSearcher {
	unsigned int _Off;
	template<typename T>
...
struct MSearcherEx {
	...
	TP* get(void* base) {
		return (TP*)(((uintptr_t)base) + myOff);
	}
};